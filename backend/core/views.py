...
def mark_order_delivered(request, order_id):
    """Supervisor confirms delivery — stock updates."""
    from django.utils import timezone
    try:
        order = FeedOrder.objects.get(id=order_id)
    except FeedOrder.DoesNotExist:
    ...
    if order.status != 'sent':
        return Response({'error': f'Order must be in "sent" status, currently: {order.status}'}, status=400)
    ...
    if not request.user.profile.is_admin and not request.user.profile.can_edit_farm(order.farm):
        return Response({'error': 'You do not have permission for this farm'}, status=403)
...
def feed_stock(request):
    """
    Current feed stock per farm.
    Stock = total delivered bags - total consumed bags (from daily entries of active flocks).
    Optional: ?farm=id
    """
    ...
    for farm in farms_qs:
        # Delivered bags
        delivered = FeedOrder.objects.filter(farm=farm, status='delivered').values('feed_type').annotate(
            total=Sum('quantity_bags')
        )
        delivered_map = {d['feed_type']: float(d['total']) for d in delivered}

        # Consumed bags (all flocks, not just active)
        ...
        # Pending orders
        pending = FeedOrder.objects.filter(farm=farm, status__in=['pending', 'sent']).values('feed_type').annotate(
            total=Sum('quantity_bags')
        )
        ...
        result.append({
            ...
            'farm_name': farm.name,
            'stock': {
                'bpsc': round(stock_bpsc, 1),
                'bsc': round(stock_bsc, 1),
                'bfp': round(stock_bfp, 1),
                'total': round(stock_bpsc + stock_bsc + stock_bfp, 1),
            },
            'delivered': {
                'bpsc': delivered_map.get('BPSC', 0),
                'bsc': delivered_map.get('BSC', 0),
                'bfp': delivered_map.get('BFP', 0),
            },
            'consumed': {
                'bpsc': consumed_bpsc,
                'bsc': consumed_bsc,
            ...
            'pending_orders': {
                'bpsc': pending_map.get('BPSC', 0),
                'bsc': pending_map.get('BSC', 0),
                'bfp': pending_map.get('BFP', 0),
            },
        })
